<?php
/**
 * cheerstheme functions and definitions
 *
...
if ( ! function_exists( 'cheerstheme_setup' ) ) :
	...
	function cheerstheme_setup() {
		 ...
		 * If you're building a theme based on cheerstheme, use a find and replace
		 * to change 'cheerstheme' to the name of your theme in all the template files.
		 */
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'cheerstheme_custom_background_args',
				array(
 ...
 */
function cheerstheme_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'cheerstheme_content_width', 640 );
}
add_action( 'after_setup_theme', 'cheerstheme_content_width', 0 );
 ...
 */
function cheerstheme_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'cheerstheme' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'cheerstheme' ),
...
add_action( 'widgets_init', 'cheerstheme_widgets_init' );
